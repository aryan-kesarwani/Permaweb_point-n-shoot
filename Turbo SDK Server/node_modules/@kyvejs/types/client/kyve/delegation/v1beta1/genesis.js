...
exports.GenesisState = {
    encode: function (message, writer) {
        ...
        for (var _i = 0, _a = message.delegator_list; _i < _a.length; _i++) {
            var v = _a[_i];
            delegation_1.Delegator.encode(v, writer.uint32(18).fork()).join();
        }
        for (var _b = 0, _c = message.delegation_entry_list; _b < _c.length; _b++) {
            var v = _c[_b];
            delegation_1.DelegationEntry.encode(v, writer.uint32(26).fork()).join();
        }
        for (var _d = 0, _e = message.delegation_data_list; _d < _e.length; _d++) {
            var v = _e[_d];
            delegation_1.DelegationData.encode(v, writer.uint32(34).fork()).join();
        }
        for (var _f = 0, _g = message.delegation_slash_list; _f < _g.length; _f++) {
            var v = _g[_f];
            delegation_1.DelegationSlash.encode(v, writer.uint32(42).fork()).join();
        }
        for (var _h = 0, _j = message.undelegation_queue_entry_list; _h < _j.length; _h++) {
            var v = _j[_h];
            delegation_1.UndelegationQueueEntry.encode(v, writer.uint32(50).fork()).join();
        }
        if (message.queue_state_undelegation !== undefined) {
            delegation_1.QueueState.encode(message.queue_state_undelegation, writer.uint32(58).fork()).join();
        }
        for (var _k = 0, _l = message.redelegation_cooldown_list; _k < _l.length; _k++) {
            var v = _l[_k];
            delegation_1.RedelegationCooldown.encode(v, writer.uint32(66).fork()).join();
        }
        return writer;
    },
